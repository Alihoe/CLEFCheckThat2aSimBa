...
class UnsupervisedFeatureSetGenerator:
    ...
    def create_sim_score(self):
        ...
        if Features.subjects.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = subjects_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = subjects_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            current_feature = current_feature * 100
            if not output_df.empty:
        ...
        if Features.main_syms_ratio.name in self.features:
            ...
            if not output_df.empty:
                ...
                sim_score = current_feature
            output_df['sim_score'] = sim_score
        if Features.main_syms_token_ratio.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = main_syms_token_ratio_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = main_syms_token_ratio_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            if not output_df.empty:
                current_sim_score = output_df['sim_score']
        ...
        if Features.words_ratio.name in self.features:
            ...
            if not output_df.empty:
                ...
                sim_score = current_feature
            output_df['sim_score'] = sim_score
        if Features.words_token_ratio.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = words_token_ratio_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = words_token_ratio_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            if not output_df.empty:
                current_sim_score = output_df['sim_score']
                ...
                sim_score = current_feature
            output_df['sim_score'] = sim_score
        if Features.ne.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = ne_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = ne_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            if not output_df.empty:
                current_sim_score = output_df['sim_score']
                ...
                sim_score = current_feature
            output_df['sim_score'] = sim_score
        if Features.main_syms.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = main_syms_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = main_syms_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            if not output_df.empty:
                current_sim_score = output_df['sim_score']
                ...
                sim_score = current_feature
            output_df['sim_score'] = sim_score
        if Features.words.name in self.features:
            if self.data == Data.pp1.name:
                sim_score_file = words_sims_test_pp1
            elif self.data == Data.TEST.name:
                sim_score_file = words_sims_test_TEST
            sim_score_df = pd.read_pickle(sim_score_file)
            current_feature = sim_score_df['sim_score']
            if not output_df.empty:
                current_sim_score = output_df['sim_score']
